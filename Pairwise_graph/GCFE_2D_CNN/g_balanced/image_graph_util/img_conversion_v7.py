...
class image_convert:
    ...
    def gen_gridlap(channel_input,patch_size_x,patch_size_y,ed0,ed1,adj_ary):
        ...
        values=v1-v2
        t=np.subtract(ed0,ed1)
        t=t.reshape(t.shape[0],1)
        edu_distance=np.absolute(np.divide(values,t))+ 10**(-8)
        revalue_s0=edu_distance.shape[0]
        revalue_s1=edu_distance.shape[1]
        re_val=edu_distance.reshape(revalue_s0,revalue_s1)
        ...
        #print(lap_a.shape)
        #print(adj_ary)
        return np.float32(lap_a)
    
    
    def to_gridlap_gersharray(self,patch_array):
    ...
    @staticmethod
    def gen_original_gridlap(channel_input,patch_size_x,patch_size_y,ed0,ed1,adj_ary):
        ch=channel_input
        n_channels=channel_input.shape[0]
        ...
        d_sum=np.sum(temp_zero,axis=(2,))
        d_zero[:, np.arange(d_zero.shape[1]), np.arange(d_zero.shape[2])] = d_sum
        #print(d_zero.shape)
        
        
        lap_a=np.subtract(d_zero,temp_zero)
    ...
    def to_original_gridlaparray(self,patch_array):
        self.patch_array=patch_array
        self.patch_length=self.patch_array.shape[0]
        self.patch_channel=self.patch_array.shape[1]
        self.patch_no=self.patch_array.shape[2]
        ...
        for k,i in enumerate(self.patch_array):
            ...
            if self.patch_channel==3:
                ch1,ch2,ch3=i
                ch1_out=image_convert.gen_original_gridlap(ch1,self.patch_size_x,self.patch_size_y,ed0,ed1,adj_ary)
                ch2_out=image_convert.gen_original_gridlap(ch2,self.patch_size_x,self.patch_size_y,ed0,ed1,adj_ary)
                ch3_out=image_convert.gen_original_gridlap(ch3,self.patch_size_x,self.patch_size_y,ed0,ed1,adj_ary)
                
                chall_out=np.array([ch1_out,ch2_out,ch3_out])
                #chall_out=np.float16(chall_out)
            else:
                ch1=i[0]
                chall_out=np.array([image_convert.gen_original_gridlap(ch1,self.patch_size_x,self.patch_size_y,ed0,ed1,adj_ary)])                                                         
                chall_out.reshape(1,self.patch_no,(self.patch_size_x*self.patch_size_x),(self.patch_size_y*self.patch_size_y))
                #chall_out=np.float16(chall_out)
